...
function getHostname() { return os.hostname().trim(); }
function getSerialNumber() {
  const platform = process.platform;
  try {
    if (platform === 'win32') {
      try { const out = execSync('wmic bios get serialnumber').toString().split('\n')[1]; if (out && out.trim()) return out.trim(); } catch {}
      try { const psOut = execSync('powershell -Command "(Get-WmiObject Win32_BIOS).SerialNumber"').toString(); if (psOut && psOut.trim()) return psOut.trim(); } catch {}
      return 'UNKNOWN_SERIAL';
    } else if (platform === 'linux') {
      try { const out = execSync('cat /sys/class/dmi/id/product_serial').toString().trim(); if (out) return out; } catch {}
      try { const out = execSync('dmidecode -s system-serial-number').toString().trim(); if (out) return out; } catch {}
      return 'UNKNOWN_SERIAL';
    } else if (platform === 'darwin') {
      try { const out = execSync("ioreg -l | awk '/IOPlatformSerialNumber/ { print $4;}'").toString(); return out.replace(/\"/g,'').trim() || 'UNKNOWN_SERIAL'; } catch {}
      return 'UNKNOWN_SERIAL';
    }
    return 'UNKNOWN_SERIAL';
...
function validateLicense() {
  ...
  if (config.machineInfo.hostname !== currentHostname || config.machineInfo.serial !== currentSerial) {
...
const forgetPassword = async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .query(`EXEC sp_user_info_hdr_sakthi @mode,'',@user_code,'','','','','','','',@email_id,'','','','','','','',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL`);
...
const Passwords = async (req, res) => {
  ...
  try {
    ...
    await pool
      ...
      .query("EXEC sp_user_info_hdr_sakthi @mode,'',@user_code,'','','',@user_password,'','','',@email_id,'','','','','','','',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL");
...
const login = async (req, res) => {
  ...
  try {
        try {
          ...
          return res.status(403).json({ success: false, message: licenseErr.message });
        }
    ...
    const result = await pool
      ...
      .query(`EXEC sp_user_info_hdr_sakthi 'LUC','',@user_code,'','','',@user_password,'','','','','','','','','','','','','','','','','','',''`);
    if (result.recordset.length > 0) {
...
const getUsercode = async (req, res) => {
  try {
    ...
    const result = await sql.query(
      "EXEC sp_user_info_hdr_sakthi 'F','','user_code','','', '' ,'','','','','','','','','','','','',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL"
    );
...
const getAlluserData = async (req, res) => {
  try {
    await connection.connectToDatabase();
    const result = await sql.query(`EXEC sp_user_info_hdr_sakthi 'A','','','','',' ','','','','','','','','','','','','',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL`);
...
const userAddData = async (req, res) => {
  ...
  try {
    ...
    if (licenseData.license.noOfUsers <= 0) {
      return res.status(400).json({ success: false, message: 'No more users can be created. License limit reached.' });
    }
    ...
    const result = await pool
      ...
      .query(`EXEC sp_user_info_hdr_sakthi @mode,@company_code,@user_code,@user_name,@first_name,@last_name,@user_password,@user_status,@log_in_out,@user_type,@email_id,@dob,@gender,@role_id,@user_img,@expiry_date,@created_by,@modified_by,@tempstr1, @tempstr2, @tempstr3, @tempstr4,@datetime1, @datetime2, @datetime3, @datetime4`);
      ...
      if (result.rowsAffected && result.rowsAffected[0] > 0) {
        ...
        return res.status(200).json({ success: true, message: 'Data inserted successfully', remainingUsers: licenseData.license.noOfUsers });
      }
  } catch (err) {
  ...
  try {
    ...
    for (const user_code of user_codesToDelete) {
        ...
        await pool.request()
          ...
          .input("company_code", sql.NVarChar, req.headers['company_code'])
          .input("modified_by", sql.NVarChar, req.headers['modified-by'])
          .query(`EXEC sp_user_info_hdr_sakthi 'D',@company_code,@user_code,'','','', 
...
const getUsersearchdata = async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .query(` EXEC sp_user_info_hdr_sakthi @mode,@company_code,@user_code,@user_name,@first_name,@last_name,'',@user_status,'','',@email_id,@dob,@gender,@role_id,@user_img,'','','','','','','','','','',''`);
...
const UpdateUserImage = async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .input("user_code", sql.NVarChar, user_code)
      .input("user_img", sql.VarBinary, user_img)
      .query(`EXEC sp_user_info_hdr_sakthi 'UI','',@user_code,'','','','','','','','','','','',@user_img,'','','',
...
const UserUpdate = async (req, res) => {
  const { company_code, user_code, user_name, first_name, last_name, user_password, user_status,
    log_in_out, user_type, email_id, dob, gender, role_id, expiry_date, created_by, modified_by
  } = req.body;
  ...
  try {
    ...
    await pool
      ...
      .input("modified_by", sql.NVarChar, modified_by)
      .query(
        `EXEC sp_user_info_hdr_sakthi @mode,@company_code, @user_code, @user_name, @first_name, @last_name, @user_password, @user_status, @log_in_out, @user_type, 
            @email_id, @dob, @gender, @role_id, @user_images, @expiry_date, @created_by, @modified_by, '', '', '', '', '', '', '', ''`);
    res.status(200).json("Edited data saved successfully");
  } catch (err) {
    console.error("Error", err);
    res.status(500).json({ message: err.message || 'Internal Server Error' });
...
const Userdropdown = async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .input("user_code",            sql.NVarChar,user_code)     
      .query(`EXEC [sp_user_info_hdr_sakthi] @mode,'',@user_code,'','','','','','','','','','','','','','','',NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL
`);
res.json(result.recordset);
} catch (err) {
...
const Fame_atten_report = async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .query(`EXEC sp_attendance_summary_report @mode,@from_date,@to_date,@dept_type`)
    // Send response
    if (result.recordset.length > 0) {
      res.status(200).json(result.recordset); // 200 OK if data is found
    } else {
...
const Fame_atten_contract = async (req, res) => {
  ...
  try {
    ...
    const result = await pool
      ...
      .query(`EXEC sp_attendance_summary_report_for_contractors @mode,@from_date,@to_date,@emp_id,@contractor_name`)
    // Send response
    if (result.recordset.length > 0) {
      res.status(200).json(result.recordset); 
    } else {
...
const getDeptType_Atte_Report = async (req, res) => {
  try {
    await connection.connectToDatabase();
    const result = await sql.query(
      "EXEC sp_attendance_get_types 'AR'"
    );
    res.json(result.recordset);
  } catch (err) {
    console.error("Error", err);
...
const getDeptType_Daily_Atte_Report = async (req, res) => {
  try {
    await connection.connectToDatabase();
    const result = await sql.query(
      "EXEC sp_attendance_get_types 'DAR'"
    );
    res.json(result.recordset);
  } catch (err) {
    console.error("Error", err);
...
const getcontractorname = async (req, res) => {
  try {
    await connection.connectToDatabase();
    const result = await sql.query(
      "EXEC sp_get_contract_details 'CN'"
    );
    res.json(result.recordset);
  } catch (err) {
    console.error("Error", err);
...
const fame_emp_details = async (req, res) => {
  try {
    await connection.connectToDatabase();
    const result = await sql.query(
      `EXEC sp_get_contract_details 'ED'`
    );
    res.json(result.recordset);
  } catch (err) {
    console.error("Error", err);
...
const TemplateInsert = async (req, res) => {
  ...
  try {
    ...
    const result = await pool
          ...
          .query(`EXEC sp_report_template @mode,@screen,@name,@settings,@gridcolumn_value,@grid_value`);
          if (result.rowsAffected && result.rowsAffected[0] > 0) {
            return res.status(200).json({ success: true, message: 'Format Saved successfully' });
          }
  } catch (err) {
      console.error("Error", err);
      res.status(500).json({ message: err.message || 'Internal Server Error' });
...
const logout = async (req, res) => {
  ...
  try {
    ...
    await pool
      ...
      .input("user_code", sql.NVarChar, user_code)
      .query(`EXEC sp_user_info_hdr_sakthi 'LOC','',@user_code,'','','','','','','','','','','','','','','','','','','','','','',''`);
...
const getLicenseDetails = async (req, res) => {
  try {
    ...
    const decryptedData = aesGcmDecryptPayload(encryptedData, CONFIG_PASSPHRASE);
...
const LeaveMasterInsert = async (req, res) => {
  const { Leave_Code,Leave_Name,Leave_Description,Leave_Type,Accrual_Flag,Accrual_Frequency,Accrual_Start_Month,TotalDaysToBeCredit,
    Max_Accrual_Days,Carry_Forward_Flag,Max_Carry_Forward_Days,Carry_Forward_Expiry_Months,Min_Leave_Apply_Days,Max_Leave_Apply_Days,
    Allow_Half_Day,Allow_Negative_Balance,Leave_Reason_Required,Document_Required,Applicable_Gender,Applicable_Employee_Type,Min_Service_Months,
    Encashment_Allowed,Encashment_Max_Days,Weekend_Count,Holiday_Count,Is_Active,Is_Deleted,Display_Order,Effective_From,Effective_To,
    Company_Code,Country_Code,Region_Code,Record_Version,Row_GUID,Created_By} = req.body;
  ...
  try {
    ...
    await pool
      ...
      .input("Max_Carry_Forward_Days", sql.Decimal(18,2), Max_Carry_Forward_Days)
      .input("Carry_Forward_Expiry_Months", sql.Int, Carry_Forward_Expiry_Months)
...
const LeaveMasterUpdate = async (req, res) => {
  const { Leave_Code,Leave_Name,Leave_Description,Leave_Type,Accrual_Flag,Accrual_Frequency,Accrual_Start_Month,TotalDaysToBeCredit,
    ...
    Allow_Half_Day,Allow_Negative_Balance,Leave_Reason_Required,Document_Required,Applicable_Gender,Applicable_Employee_Type,Min_Service_Months,
    Encashment_Allowed,Encashment_Max_Days,Weekend_Count,Holiday_Count,Is_Active,Is_Deleted,Display_Order,Effective_From,Effective_To,
    Company_Code,Country_Code,Region_Code,Record_Version,Row_GUID,Keyfield,Modified_By} = req.body;
...
const LeaveMasterSearch = async (req, res) => {
  const { Leave_Code,Leave_Name,Leave_Description,Min_Leave_Apply_Days,Max_Leave_Apply_Days,
  Is_Active,Effective_From,Effective_To,Company_Code} = req.body;
  ...
  try {
    ...
    for (const updatedRow of editedData) {
      ...
      await pool
        ...
        .input("Min_Leave_Apply_Days", sql.Decimal(18, 2), updatedRow.Min_Leave_Apply_Days)
        .input("Max_Leave_Apply_Days", sql.Decimal(18, 2), updatedRow.Max_Leave_Apply_Days)
